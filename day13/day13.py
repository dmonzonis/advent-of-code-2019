...
def parse_tilemap(outputs):
    """Returns a dictionary with positions (pairs of x,y) as keys and tile type as values."""
    i = 0
    result = {}
    while i + 2 < len(outputs):
        result[(outputs[i], outputs[i + 1])] = outputs[i + 2]
        i += 3
    return result
...
def main():
    ...
    cpu = Intcode(data)
    cpu.execute()
    tilemap = parse_tilemap(cpu.outputs)
    print("Part 1:", sum(val == 2 for val in tilemap.values()))
